...
description: "[Android App Development] [Claude Code] 안드로이드 최초 기획 문서(PRD, TRD 등)를 리뷰하고 안드로이드 생태계 제약사항에 맞춰 정합성을 교정, 문서를 상세화하는 안드로이드 메인 Agent 스킬입니다."
context: fork
---
...
이 Agent는 표준 순서 `pipeline-orchestrator -> document-review -> guide-generation -> implementation -> review`에서 **첫 번째 worker 단계**입니다.

*   **upstream:** `pipeline-orchestrator`
*   **downstream:** worker 관점에서 다음 단계는 `guide-generation`이지만, 실제 dispatch 결정은 `pipeline-orchestrator`가 수행
...
세부 필드와 루프 원칙은 `skills/pipeline-orchestrator-agent/agent-session-contract.md`를 기준으로 합니다.
...
> **⚠️ CRITICAL:** 아래 템플릿의 **모든 키는 필수**입니다. 하나라도 누락되면 파이프라인 검증이 실패합니다. 섹션 제목은 반드시 `## Session Update - Document Review` 형식(h2 + "Session Update -" 접두사)을 사용해야 합니다.
...
- **next_agent_focus:** [guide-generation이 집중해야 할 포인트]
...
> **⚠️ CRITICAL:** 아래 템플릿의 **모든 키는 필수**입니다. 특히 `completed_agent`, `updated_documents`, `corrected_inconsistency_count`, `context_snapshot_path`, `next_agent_context`를 절대 누락하지 마세요. 하나라도 빠지면 파이프라인 검증이 실패합니다.
